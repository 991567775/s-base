                            ...
                            if (name.startsWith(packageDirName)) {
                                ...
                                if ((idx != -1) || recursive) {
                                    ...
                                    if (name.endsWith(".class") && !entry.isDirectory()) {
                                        ...
                                        try {
                                            // 添加到classes
                                            final Class<?> aClass = Class.forName(packageName + '.' + className);
                                            if(aClass.isAnnotationPresent(configuration.class)
                                                    ||aClass.isAnnotationPresent(bean.class)||aClass.isAnnotationPresent(client.class)
                                                   || className.endsWith(ZdConst.end_with_service_impl)
                                                    || className.endsWith(ZdConst.end_with_control)|| className.endsWith(ZdConst.end_with_Dao)){
                                                classes.add(aClass);
                                            }
